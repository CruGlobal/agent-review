...
// engine/upgradeConfig.cjs
var require_upgradeConfig = __commonJS({
  "engine/upgradeConfig.cjs"(exports2, module2) {
    "use strict";
    var LEGACY = {
...
var require_scoreRisk = __commonJS({
  "engine/scoreRisk.cjs"(exports2, module2) {
    ...
    function scoreRisk({ files, linesChanged, scope = "single_feature", special = [] }, config) {
      ...
      const scopeMultiplier = config.risk.scope_multiplier[scope] ?? 1;
...
// engine/selectAgents.cjs
var require_selectAgents = __commonJS({
...
// engine/resolveRules.cjs
var require_resolveRules = __commonJS({
  "engine/resolveRules.cjs"(exports2, module2) {
    "use strict";
    var { minimatch } = require_commonjs3();
    var OPTS = { dot: true };
    ...
    module2.exports = { resolveRules };
  }
});

// engine/detectSpecial.cjs
var require_detectSpecial = __commonJS({
  "engine/detectSpecial.cjs"(exports2, module2) {
    "use strict";
    var { minimatch } = require_commonjs3();
    var OPTS = { dot: true };
    function escapeRe(s) {
    ...
    function detectSpecial(diffText, changedFiles, config) {
      ...
      if (sec && changedFiles.some((f) => (sec.files || []).some((g) => minimatch(f, g, OPTS))) && (sec.keywords || []).length && new RegExp(`(${(sec.keywords || []).map(escapeRe).join("|")})`, "i").test(diffText))
        found.add("config_security_change");
      return [...found];
    }
    module2.exports = { detectSpecial };
  }
});
...
var require_plan = __commonJS({
  "engine/plan.cjs"(exports2, module2) {
    ...
    var { loadConfig } = require_loadConfig();
    var { scoreRisk } = require_scoreRisk();
...
// engine/resolveImport.cjs
var require_resolveImport = __commonJS({
  "engine/resolveImport.cjs"(exports2, module2) {
    "use strict";
    var path = require("node:path");
    var DEFAULT_ALIASES = ["src/", "pages/", "__tests__/"];
    var DEFAULT_EXTS = [".ts", ".tsx", ".d.ts", ".js", ".jsx", ".json"];
    ...
    function resolveImport(fromFile, spec, fileSet, opts = {}) {
      ...
      const norm = aliases.map(
        (a) => typeof a === "string" ? { prefix: a, target: a } : a
      );
      let base;
      const hit = norm.find(
...
// engine/buildGraph.cjs
var require_buildGraph = __commonJS({
  "engine/buildGraph.cjs"(exports2, module2) {
    "use strict";
    var { resolveImport } = require_resolveImport();
    var PATTERNS = [
      /\bimport\b[^'"]*?\bfrom\s*['"]([^'"]+)['"]/g,
      /\bimport\s*['"]([^'"]+)['"]/g,
      /\bexport\b[^'"]*?\bfrom\s*['"]([^'"]+)['"]/g,
    ...
    module2.exports = { buildGraph, extractSpecifiers };
  }
});

// engine/indexStore.cjs
var require_indexStore = __commonJS({
  "engine/indexStore.cjs"(exports2, module2) {
    "use strict";
    var {
      readFileSync,
      writeFileSync,
    ...
    var { buildGraph } = require_buildGraph();
    var { DEFAULT_EXTS } = require_resolveImport();
    var { parseArgs } = require_args();
    var DEFAULT_ROOTS = ["src", "pages", "__tests__"];
    function indexRegex(roots, exts) {
      const r = (roots && roots.length ? roots : DEFAULT_ROOTS).map((s) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|");
      const e = (exts && exts.length ? exts : DEFAULT_EXTS).map((x) => x.replace(/^\./, "").replace(/\./g, "\\.")).join("|");
    ...
    function csv(v) {
      return typeof v === "string" ? v.split(",").map((s) => s.trim()).filter(Boolean) : void 0;
    }
    if (require.main === module2) {
      const a = parseArgs(process.argv.slice(2));
...
// engine/queryImpact.cjs
var require_queryImpact = __commonJS({
  "engine/queryImpact.cjs"(exports2, module2) {
    "use strict";
    function queryImpact(changedFiles, graph, opts = {}) {
      ...
      for (const f of changedFiles) {
        directDependents[f] = (importedBy[f] || []).filter(
          (d) => !changedSet.has(d)
        );
      }
      ...
      const topImpacted = changedFiles.map((f) => ({ file: f, dependentCount: directDependents[f].length })).sort((a, b) => b.dependentCount - a.dependentCount);
      return {
        directDependents,
    ...
    module2.exports = { queryImpact };
  }
});

// engine/findingSignature.cjs
var require_findingSignature = __commonJS({
  "engine/findingSignature.cjs"(exports2, module2) {
    "use strict";
    var { createHash } = require("node:crypto");
    function normalizeMessage(msg) {
      return String(msg || "").toLowerCase().replace(/['"`][^'"`]*['"`]/g, " ").replace(/\d+/g, " ").replace(/\s+/g, " ").trim();
    }
    ...
    module2.exports = { signature, normalizeMessage, topDir };
  }
});

// engine/mineLearnings.cjs
var require_mineLearnings = __commonJS({
  "engine/mineLearnings.cjs"(exports2, module2) {
    "use strict";
    var { topDir } = require_findingSignature();
      ...
      for (const [sig, entries] of groups) {
        ...
        if (dismissed / total >= 0.75) {
          proposals.push({
            ...
            kind: "suppress",
            status: "proposed",
            rationale: `Dismissed ${dismissed}/${total} times`
          });
          ...
          proposals.push({
            ...
            kind: "rule",
            status: "proposed",
            ruleText: sample.message,
            rationale: `Accepted ${accepted}/${total} times`
          });
        }
      }
      return proposals;
    }
    module2.exports = { mineLearnings };
  }
});
...
var require_applyLearnings = __commonJS({
  "engine/applyLearnings.cjs"(exports2, module2) {
    ...
    function filterFindings(findings, approved) {
      const sigs = new Set(
        (approved || []).filter((l) => l.kind === "suppress").map((l) => l.signature)
      );
      const kept = [];
      const suppressed = [];
    ...
    function rulesFromLearnings(approved) {
      return (approved || []).filter((l) => l.kind === "rule").map((l) => ({
        paths: l.paths || [],
        ruleText: l.ruleText || l.example || "",
        agent: l.agent
      }));
    }
    module2.exports = { filterFindings, rulesFromLearnings };
  }
});

// engine/paths.cjs
var require_paths = __commonJS({
  "engine/paths.cjs"(exports2, module2) {
    "use strict";
    var { join, isAbsolute, resolve } = require("node:path");
    ...
    module2.exports = { repoRoot, reviewDir };
  }
});

// engine/learningsStore.cjs
var require_learningsStore = __commonJS({
  "engine/learningsStore.cjs"(exports2, module2) {
    "use strict";
    var {
      readFileSync,
      writeFileSync,
    ...
    function mergeProposals(existing, proposals) {
      const out = {
        version: 1,
        learnings: [...existing && existing.learnings || []]
      };
    ...
    if (require.main === module2) {
      const { parseArgs } = require_args();
      const { reviewDir } = require_paths();
      const a = parseArgs(process.argv.slice(2));
      const base = a["learning-dir"] || join(reviewDir({ root: a.root, reviewDir: a["review-dir"] }), "learnings");
...
`
        ...
        const entries = parsePending(readFileSync(pendingFile, "utf8")).map(
          (e) => ({ ts: (/* @__PURE__ */ new Date()).toISOString(), ...e })
        );
...
`
        ...
        process.stdout.write(
          JSON.stringify(
            rulesFromLearnings(loadApproved(loadLearnings(learningsPath))),
            null,
            2
          ) + "\n"
        );
        ...
        const raw = JSON.parse(
          readFileSync(typeof a.in === "string" ? a.in : findingsPath, "utf8")
        );
        process.stdout.write(
          JSON.stringify(
            filterFindings(
              raw.findings || raw,
              loadApproved(loadLearnings(learningsPath))
            ),
            null,
            2
          ) + "\n"
        ...
        process.stdout.write(
          "usage: learningsStore.cjs [--emit --in <json> --review <id> | --ingest <pending.yml> | --mine [--min-support N] | --rules | --filter --in <json>]\n"
        );
      }
    }
  }
...
var require_reportState = __commonJS({
  "engine/reportState.cjs"(exports2, module2) {
    ...
    module2.exports = { findingList, cleanPrevious, mergeLedger, buildStatus };
  }
});
...
var require_addressState = __commonJS({
  "engine/addressState.cjs"(exports2, module2) {
    ...
    var { createHash } = require("node:crypto");
    var { DISMISSAL_REASONS } = require_evalSuite();
    var { cleanPrevious } = require_reportState();
    var RESULT_STATUSES = /* @__PURE__ */ new Set(["applied", "not-applied"]);
    var TEST_STATUSES = /* @__PURE__ */ new Set(["passed", "failed", "not-run"]);
    var MAX_PATCH_BYTES = 1024 * 1024;
    var MAX_RESULT_BYTES = 256 * 1024;
    var MAX_CHANGED_FILES = 25;
    var SENSITIVE_PATHS = [
      ".github/workflows/",
      ".claude/",
      ".gitmodules",
    ...
    function singleLine(value, field, max = 1e3) {
    ...
    function safeRepoPath(value, field = "path") {
      ...
      if (!path || path.length > 500 || path.startsWith("/") || path.startsWith("-") || path.includes("\\") || /[\r\n\0]/.test(path) || path.split("/").some((part) => part === "" || part === "." || part === "..")) {
    ...
    function parseNumberList(value) {
      return String(value).split(",").map((part) => Number(part.trim().replace(/^#/, ""))).map((n) => {
    ...
    function parseCommand(body) {
      ...
      for (const clause of command.split(/\s*;\s*/)) {
        ...
        throw new Error(
          'invalid address syntax; use "@claude fix 1, 3" or "@claude dismiss 2 [false-positive]: reason" and separate mixed clauses with ";"'
        );
      }
      const seen = /* @__PURE__ */ new Set();
      ...
      return {
        ...
        operations: request.operations.map((raw) => {
          ...
          const operation = {
            ...
            message: singleLine(raw.message, `finding #${n} message`, 2e3)
          };
    ...
    function isSensitivePath(path) {
      return SENSITIVE_PATHS.some(
        (sensitive) => sensitive.endsWith("/") ? path.startsWith(sensitive) : path === sensitive
      );
    }
    function validateAddressResult({ request: requestInput, result: resultInput, changedFiles, patchBytes }) {
      ...
      const fixes = result.fixes.map((raw) => {
        ...
        if (raw.status === "applied") {
          ...
          clean.summary = singleLine(raw.summary, `finding #${n} summary`, 1e3);
        } else {
          clean.reason = singleLine(raw.reason, `finding #${n} not-applied reason`, 1e3);
          clean.files = [];
        }
        return clean;
      });
      ...
      for (const path of changedSet) {
        ...
        if (isSensitivePath(path) && !request.operations.some((operation) => operation.action === "fix" && operation.file === path)) {
      ...
      const tests = result.tests.map((raw, index) => {
        ...
        return {
          ...
          status: raw.status,
          ...raw.details ? { details: singleLine(raw.details, "test details", 1e3) } : {}
        };
      });
      return { version: 1, expectedHead: request.expectedHead, fixes, tests };
    }
    function feedbackForAddress(requestInput, resultInput, now = (/* @__PURE__ */ new Date()).toISOString()) {
      const request = validateRequest(requestInput);
      ...
      return request.operations.flatMap((operation) => {
        ...
        return [{
          ...
          ...operation.action === "dismiss" ? { dismissalReason: operation.reasonCode, dismissalDetail: operation.reason } : {}
        }];
      });
    }
...
var require_cliCommands = __commonJS({
  "engine/cliCommands.cjs"(exports2, module2) {
    ...
    function setLearningStatus(learnings, id, status) {
      ...
      return {
        ...learnings,
        learnings: list.map((l) => l.id === id ? { ...l, status } : l)
      };
    }
    function listLearnings(learnings, statusFilter) {
      const list = learnings && learnings.learnings || [];
      return list.filter((l) => !statusFilter || l.status === statusFilter).map((l) => ({
        id: l.id,
        kind: l.kind,
        status: l.status,
        support: l.support,
    ...
    function preflightSummary(plan, impact) {
      ...
      if (impact) {
        lines.push(
          `impact: blastRadius ${impact.blastRadius}${impact.truncated ? " (truncated)" : ""}`
        );
    ...
    module2.exports = { setLearningStatus, listLearnings, preflightSummary };
  }
});

// engine/cli.cjs
var require_cli = __commonJS({
  "engine/cli.cjs"(exports2, module2) {
    "use strict";
    var { join, relative, dirname } = require("node:path");
    var { execFileSync } = require("node:child_process");
    var {
      readFileSync,
      writeFileSync,
    ...
    var { createHash } = require("node:crypto");
    var { loadConfig } = require_loadConfig();
    var { buildPlan, linesChangedFromStat } = require_plan();
    var {
      loadOrBuildIndex,
      gitHead,
      listRepoFiles
    } = require_indexStore();
    var { queryImpact } = require_queryImpact();
    var { mineLearnings } = require_mineLearnings();
    var {
      parsePending,
      appendFeedback,
    ...
    var { repoRoot, reviewDir } = require_paths();
    function out(s) {
    ...
    function planTmpPath(root) {
      return join(
        os.tmpdir(),
        "agent-review-plan-" + createHash("sha1").update(root).digest("hex").slice(0, 12) + ".json"
      );
    }
    ...
    function loadIndex(C, cfg, { force } = {}) {
      ...
      const indexPath = c.index && c.index.path ? join(C.ROOT, c.index.path) : C.INDEX;
      if (force) {
        const gf = join(indexPath, "graph.json");
        if (existsSync(gf)) rmSync(gf);
      }
    ...
    var USAGE = `usage: agent-review <command>
      ...
      switch (cmd) {
        case "config": {
          ...
          if (rest[0] === "get") {
            ...
            const val = rest[1].split(".").reduce((o, k) => o == null ? void 0 : o[k], cfg);
            out(
        ...
        case "index": {
          const g = loadIndex(C, void 0, { force: rest.includes("--force") });
          out(
        ...
        case "run": {
          ...
          const impact = cfg.index && cfg.index.enabled ? queryImpact(files, loadIndex(C, cfg), {}) : null;
...
// engine/bin-entry.cjs
var { main } = require_cli();
try {
