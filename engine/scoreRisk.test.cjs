...
const config = {
  risk: {
    patterns: [
      {
        glob: 'src/lib/apollo/{client,link,cache,ssrClient}.ts',
        points: 3,
        tier: 'critical',
      },
...
test('UI-only small change scores LOW', () => {
  const r = scoreRisk(
    { files: ['src/components/Tasks/TaskRow.tsx'], linesChanged: 30 },
    config,
  );
  ...
  const r = scoreRisk(
    {
      files: [
        ...
        'pages/api/Schema/Foo/resolvers.ts',
      ],
      linesChanged: 600,
      scope: 'cross_cutting',
...
test('excluded files do not contribute to score', () => {
  const r = scoreRisk(
    { files: ['__snapshots__/x.snap'], linesChanged: 10 },
    config,
  );
