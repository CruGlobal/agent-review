...
test('#2 selectAgents: content triggers ignore keywords inside markdown/excluded files', () => {
  ...
  assert.deepEqual(
    selectAgents(
      {
        files: ['.claude/review/rules/financial.md', 'docs/notes.md'],
        diffText: docDiff,
      },
      config,
    ),
  ...
  assert.ok(
    selectAgents({ files: ['src/x.ts'], diffText: codeDiff }, config).some(
      (a) => a.id === 'financial',
    ),
  );
});
...
test("#2 selectAgents: the reviewer's own config.yml trigger vocabulary does not self-select agents", () => {
  ...
  const diff =
    'diff --git a/.claude/review/config.yml b/.claude/review/config.yml\n+      content: ["amount", "currency"]';
  assert.deepEqual(
    selectAgents(
      { files: ['.claude/review/config.yml'], diffText: diff },
      config,
    ),
    [],
  );
...
test('#22 detectSpecial: negative/boundary cases do not over-flag', () => {
  const config = {
    risk: {
      special: [{ when: 'critical_pkg_update', points: 3, packages: ['next'] }],
    },
  };
  assert.ok(
    !detectSpecial('-    "lodash": "^4",', ['package.json'], config).includes(
      'new_dependency',
    ),
  );
  assert.ok(
    !detectSpecial('+ x', ['yarn.lock', 'package.json'], config).includes(
      'lockfile_only_change',
    ),
  );
  assert.ok(
    !detectSpecial('+ const x = 1;', ['next.config.ts'], config).includes(
...
test('#12 loadConfig: throws a friendly error on an invalid config', () => {
  ...
  assert.throws(
    () => loadConfig({ configPath: cfgPath, schemaPath }),
...
test('#23 resolveImport: honors config-driven aliases + extensions', () => {
  ...
  assert.equal(
    resolveImport('x.ts', 'app/foo', fileSet, {
      aliases: ['app/', 'lib/'],
      exts: ['.mjs', '.ts'],
    }),
    'app/foo.mjs',
  ...
  assert.equal(
    resolveImport('x.ts', 'src/foo', fileSet, {
      aliases: ['app/'],
      exts: ['.ts'],
    }),
    null,
